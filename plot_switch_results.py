...
def _choose_unit(vmax_mw: float, unit: str, pretty_min=0.1, pretty_max=1000.0):
    ...
    for name, mult in [("TW",1e-6),("GW",1e-3),("MW",1.0),("kW",1e3),("W",1e6)]:
...
def load_color_map_from_inputs(inputs_dir):
    ...
    if not os.path.exists(colors_csv):
        print("[warn] graph_tech_colors.csv not found; colors will use matplotlib defaults.")
        return {}
    ...
    if type_col is None or color_col is None:
        print("[warn] graph_tech_colors.csv lacks recognizable columns; using defaults.")
        return {}

    mapping = (
        df[[type_col, color_col]]
        .dropna()
        .astype(str)
        .drop_duplicates()
        .values.tolist()
...
def load_tech_to_type_map(inputs_dir):
    ...
    if not os.path.exists(types_csv):
        print("[warn] graph_tech_types.csv not found; will fall back to gen_tech as category.")
        return {}
...
def dispatch_matrix():
    ...
    disp = (disp.groupby("timestamp", as_index=False)["DispatchGen_MW"]
                .sum())
    ...
    prof = (
        disp.groupby(["day_index", "hour_of_day"], as_index=False)
            .agg(sum_mw_w=("mw_w", "sum"), sum_w=("w", "sum"))
    )
    prof["Dispatch_GW"] = prof["sum_mw_w"] / prof["sum_w"]
    prof = prof[["day_index", "hour_of_day", "Dispatch_GW"]]

    # Pivot to day × hour matrix
    pvt = (prof.pivot(index="day_index", columns="hour_of_day", values="Dispatch_GW")
                .sort_index())
...
def dispatch_matrix_per_period():
    ...
    disp = _csv(os.path.join(outputs_dir, "dispatch.csv"),
                usecols=["timestamp", "DispatchGen_MW"])
    ...
    series_list = (
        disp["timeseries"].dropna().drop_duplicates().tolist()
    )
    ...
    for s in series_list:
        ...
        pvt = (g.pivot(index="day_index", columns="hour_of_day", values="DispatchGen_MW")
    ...
    cbar = fig.colorbar(mappable, ax=axes, orientation="vertical", pad=0.01, fraction=0.02)
    cbar.set_label("GW")

    fig.suptitle(_wrap(f"{scenario_name}: Average daily dispatch by representative week"), y=0.995)
...
def dispatch_by_region(
    ...
    tech_col = "gen_type" if "gen_type" in df.columns else "gen_tech" if "gen_tech" in df.columns else None
    if tech_col is None:
    ...
    if tech_col == "gen_tech" and "gen_type" not in disp.columns:
        ...
        if os.path.exists(type_path):
            ...
            tech_c = lower.get("gen_tech") or lower.get("technology") or lower.get("tech")
            ...
            if tech_c and type_c:
                ...
                disp["gen_type"] = disp["gen_tech"].map(tech_to_type).fillna(disp["gen_tech"])
            else:
    ...
    disp["DispatchGen_MW"] = pd.to_numeric(disp["DispatchGen_MW"], errors="coerce").fillna(0.0)
    ...
    series_col = ts_lower.get("timeseries", "timeseries")
    scale_col  = ts_lower.get("ts_scale_to_period", "ts_scale_to_period")
    weights = ts[[series_col, scale_col]].rename(columns={series_col: "timeseries", scale_col: "ts_scale_to_period"})
    weight_map = weights.set_index("timeseries")["ts_scale_to_period"].to_dict()
    ...
    if os.path.exists(color_path):
        ...
        color_map = dict(zip(color_df[key_col].astype(str), color_df[col_col].astype(str)))
    else:
        color_map = {}
    ...
    target_zones = all_zones if zones in ("all", None) else [z for z in ([zones] if isinstance(zones, str) else zones) if z in all_zones]
    ...
    def pivot_hourly(df_in):
        p = (df_in.groupby(["hour_abs", "gen_type"], as_index=False)["Dispatch_units"].sum()
    ...
    for zone in target_zones:
        ...
        if split_weeks:
            ...
            for i, (ax, s) in enumerate(zip(axes_flat, series_list)):
                ...
                if plot_style == "stacked":
                    ...
                    for c in p.columns:
                        ax.plot(p.index.values, p[c].values, lw=1.2, color=color_for_type(c), label=c)
                        used_types.add(c)
                w = float(weight_map.get(s, 1.0))
                pct = 100.0 * w / total_w
            ...
            handles = [Patch(facecolor=color_for_type(c), edgecolor="none") for c in legend_types]
            labels = [str(c) for c in legend_types]
            fig.legend(handles, labels, loc="center right", bbox_to_anchor=(1.05, 0.5),
                       frameon=False, title="Generation Type")

            fig.suptitle(f"{scenario_name}: {zone} — Dispatch by representative week "
                         f"({'stacked' if plot_style == 'stacked' else 'lines'})", y=0.995)
        ...
        # ---------------- Weighted-average representative week ----------------
            ...
            prof = (g.groupby(["hour_abs", "gen_type"], as_index=False)
                     .agg(sum_u_w=("units_w", "sum"), sum_w=("w", "sum")))
            prof["Dispatch_units"] = np.divide(prof["sum_u_w"], prof["sum_w"],
                                               out=np.zeros_like(prof["sum_u_w"]), where=prof["sum_w"] > 0)
            p = (prof.pivot(index="hour_abs", columns="gen_type", values="Dispatch_units")
                     .reindex(index=np.arange(0, 168)).fillna(0.0))
            ...
            if plot_style == "stacked":
                ...
                for c in p.columns:
                    ax.plot(p.index.values, p[c].values, lw=1.6, color=color_for_type(c), label=c)

            _set_week_ticks(ax)
            ax.set_xlabel("Day of representative week")
            ax.set_ylabel(out_unit)
            ax.set_title(_wrap(f"[Dispatch] {scenario_name}: {zone} — Average daily dispatch ({out_unit})"))
...
def generation_mix_stacked_bars(
    ...
    if "gen_type" not in df.columns:
        if "gen_tech" not in df.columns:
            raise ValueError("Missing 'gen_type' and 'gen_tech'; cannot map technologies to types.")
        df = map_gen_type(df)

    # Remove duplicated capacity columns that are irrelevant here
    dup_cols = [c for c in df.columns if c.startswith("GenCapacity_MW") and c != "GenCapacity_MW"]
    if dup_cols:
    ...
    if bucket_small and len(agg) > 0:
        ...
        for p in periods:
            ...
            if small_mask.any():
                ...
                rows.append(
                    pd.concat(
                        [big, pd.DataFrame([{"period": p, "gen_type": "Other", energy_col: other_val}])],
                        ignore_index=True,
                    )
                )
            else:
    ...
    cycle = plt.rcParams.get("axes.prop_cycle").by_key().get("color", [])
    colors = [("lightgray" if c == "Other" else color_for(c, cycle)) for c in pivot.columns]
    ...
    for col, col_color in zip(pivot.columns, colors):
        ...
        bars = ax.bar(x, vals, width=width, bottom=bottom, color=col_color, edgecolor="none")
        bars_by_cat.append(bars)

        if annotate_bars:
            shares = np.divide(vals, period_totals, out=np.zeros_like(vals), where=period_totals > 0)
            for xi, v, btm, sh in zip(x, vals, bottom, shares):
                if v > 0 and sh >= annotate_threshold:
                    ax.text(
                        xi, btm + v * 0.5, f"{sh*100:.0f}%",
                        ha="center", va="center", fontsize=FIG_FONT_SIZE - 2
                    )
    ...
    ax.legend(
        ...
        ncol=ncols_legend, title="Generation Type · Energy Share", frameon=False
    )
...
def generation_mix_stacked_bars_by_zone(
    ...
    if not os.path.exists(path):
        raise FileNotFoundError("dispatch_gen_annual_summary.csv not found in outputs_dir.")
    df = _csv(path)
    ...
    if missing:
        raise ValueError(f"dispatch_gen_annual_summary.csv is missing columns: {sorted(missing)}")
    ...
    agg_all = (
        df.groupby(["period", "gen_load_zone", "gen_type"], as_index=False)[energy_col]
          .sum()
    )
    ...
    def _prep_legend_and_order(scope_df):
        ...
        totals_ser = (scope_df.groupby("gen_type", as_index=False)[energy_col]
                               .sum().set_index("gen_type")[energy_col])
    ...
    if facet_periods and len(sel_periods) > 1:
        ...
        zones_in_sel = sorted(agg_all[agg_all["period"].isin(sel_periods)]["gen_load_zone"].unique().tolist())
        ...
        for ax, p in zip(axes, sel_periods):
            ...
            if bucket_small:
                ...
                for z, zdf in sub.groupby("gen_load_zone", as_index=False):
                    ...
                    if total <= 0:
                        rows.append(zdf[["period", "gen_load_zone", "gen_type", energy_col]])
                        continue
                    zdf = zdf.copy()
                    zdf["share"] = zdf[energy_col] / total
                    small = zdf["share"] < float(bucket_threshold)
                    big = zdf.loc[~small, ["period", "gen_load_zone", "gen_type", energy_col]]
                    if small.any():
                        other_val = zdf.loc[small, energy_col].sum()
                        rows.append(pd.DataFrame(
                            [{"period": p, "gen_load_zone": z, "gen_type": "Other", energy_col: other_val}]
            ...
            pivot = (sub.pivot(index="gen_load_zone", columns="gen_type", values=energy_col)
            ...
            # Plot bars
            x = np.arange(len(pivot)); width = 0.8
            bottom = np.zeros(len(pivot), dtype=float)
            zone_totals_for_share = pivot.sum(axis=1).values

            for col in pivot.columns:
                vals = pivot[col].values
                ax.bar(x, vals, width=width, bottom=bottom, color=_col_for(col), edgecolor="none")
                if annotate_bars:
                    shares = np.divide(vals, zone_totals_for_share, out=np.zeros_like(vals), where=zone_totals_for_share > 0)
                    for xi, v, btm, sh in zip(x, vals, bottom, shares):
                        if v > 0 and sh >= annotate_threshold:
                            ax.text(xi, btm + v * 0.5, f"{sh*100:.0f}%", ha="center", va="center", fontsize=FIG_FONT_SIZE - 2)
                bottom += vals
        ...
        handles = [matplotlib.patches.Patch(facecolor=_col_for(c), edgecolor="none") for c in leg_cats]
        fig.legend(handles, leg_labels, loc="center right", bbox_to_anchor=(1.02, 0.5),
                   frameon=False, title="Generation Type · Totals")

        fig.suptitle(_wrap(f"{scenario_name}: Zonal generation mix ({unit}) — Periods {sel_periods[0]}–{sel_periods[-1]}"), y=0.995)

        # out_dir = os.path.join(outputs_dir, SAVE_SUBDIR)
        out_dir = _outdir("generation", "by_zone")
        os.makedirs(out_dir, exist_ok=True)
        fname = _safe_filename(f"generation_mix_by_zone_periods_{sel_periods[0]}_{sel_periods[-1]}_{scenario_name}")
        # fig.savefig(os.path.join(out_dir, fname), bbox_inches="tight")
        fig.savefig(os.path.join(out_dir, fname), bbox_inches="tight")
    ...
    for p in sel_periods:
        sub = agg_all[agg_all["period"] == p].copy()
        if sub.empty:
            fig = plt.figure(); fig.set_size_inches(12, 4)
            ax = fig.add_subplot(111)
            ax.text(0.5, 0.5, f"No energy by zone for period {p}.", ha="center", va="center")
            ax.axis("off")
            ...
            for z, zdf in sub.groupby("gen_load_zone", as_index=False):
                ...
                if small.any():
                    ...
                    rows.append(pd.concat(
                        [big, pd.DataFrame([{"period": p, "gen_load_zone": z, "gen_type": "Other", energy_col: other_val}])],
                        ignore_index=True
                    ))
                else:
                    rows.append(big)
        ...
        pivot = sub.pivot(index="gen_load_zone", columns="gen_type", values=energy_col).fillna(0.0)
        ...
        ax.legend(handles, legend_labels, loc="center left", bbox_to_anchor=(1.02, 0.5),
                  ncol=ncols_legend, title="Generation Type · Totals", frameon=False)
...
def curtailment_matrix(
    ...
    if disp.empty:
        fig = plt.figure(); fig.set_size_inches(10, 4)
        ax = fig.add_subplot(111)
    ...
    if aggregate == "week":
        ...
        disp = disp.merge(weights_df, on="timeseries", how="left", validate="many_to_one")
        disp["ts_scale_to_period"] = disp["ts_scale_to_period"].fillna(1.0)
        ...
        for src in panels:
            ...
            d = d.groupby(["timeseries", "timestamp", "day_index", "hour_of_day"], as_index=False)["Curtailment_MW"].sum()
            ...
            prof = (
                d.groupby(["day_index", "hour_of_day"], as_index=False)
                 .agg(sum_mw_w=("mw_w", "sum"), sum_w=("w", "sum"))
            )
            prof["Curtailment_MW"] = np.divide(
                prof["sum_mw_w"], prof["sum_w"],
                out=np.zeros_like(prof["sum_mw_w"]), where=prof["sum_w"] > 0
            )

            pvt = (prof.pivot(index="day_index", columns="hour_of_day", values="Curtailment_MW")
                        .reindex(index=range(0, 7), columns=range(0, 24))).fillna(0.0)
            mat = pvt.values
        ...
        nrows = len(panels)
        fig = plt.figure(); fig.set_size_inches(14, 4.5 * nrows)
        axes = [fig.add_subplot(nrows, 1, i + 1) for i in range(nrows)]

        for ax, (label, mat) in zip(axes, mats):
            ax.set_title(_wrap(f"[Curtailment] {scenario_name}: Average daily curtailment ({out_unit})"))
            im = ax.imshow(mat, aspect="auto", origin="lower", vmin=vmin, vmax=vmax)
            ttl = (f"{scenario_name}: Average daily curtailment ({out_unit})"
                   if label == "Solar+Wind" else
                   f"{scenario_name}: {label} curtailment ({out_unit})")
            ax.set_title(_wrap(ttl))
            ax.set_ylabel("Day")
            ax.set_yticks(range(0, 7)); ax.set_yticklabels([f"{d+1}" for d in range(7)])
    ...
    if aggregate == "split_week":
        ...
        for s in series_list:
            ...
            for src in panel_specs:
                d = g_ts if src == "Solar+Wind" else g_ts[g_ts["gen_energy_source"] == src]
                d = d.groupby(["timestamp", "day_index", "hour_of_day"], as_index=False)["Curtailment_MW"].sum()
                pvt = (d.pivot(index="day_index", columns="hour_of_day", values="Curtailment_MW")
                         .reindex(index=range(0, 7), columns=range(0, 24))).fillna(0.0)
                mat = pvt.values
        ...
        if not split_by_source:
            ...
            for i, s in enumerate(series_list):
                ...
                mat = next(m for (ts_name, src_name, m) in mats if ts_name == s and src_name == "Solar+Wind")
                im = ax.imshow(mat, aspect="auto", origin="lower", vmin=vmin, vmax=vmax)
                last_im = im
                w = float(weight_map.get(s, 1.0)); pct = 100.0 * w / total_w
                ax.set_ylabel(f"{s}\n{int(w)} wk ({pct:.1f}%)", rotation=0, labelpad=45,
                              ha="right", va="center", fontsize=max(8, FIG_FONT_SIZE-1))
                ax.set_yticks(range(0, 7)); ax.set_yticklabels([f"{d+1}" for d in range(7)])
                ax.set_xticks(range(0, 24, 4)); ax.set_xticklabels([str(h) for h in range(0, 24, 4)])
            ...
            fig.suptitle(_wrap(f"{scenario_name}: Average daily curtailment by representative week"),
                         y=0.995)
        else:
            # Two panels per timeseries (Solar|Wind) side-by-side; wrap after max_rows_per_col
            ncols_groups = math.ceil(n_ts / max_rows_per_col)  # number of TS column groups
            nrows = min(max_rows_per_col, n_ts)
            ...
            for i, s in enumerate(series_list):
                ...
                # Solar
                mat_s = next(m for (ts_name, src_name, m) in mats if ts_name == s and src_name == "Solar")
                ax_s = axes[row, col_solar]
                im = ax_s.imshow(mat_s, aspect="auto", origin="lower", vmin=vmin, vmax=vmax)
                last_im = im

                # Wind
                mat_w = next(m for (ts_name, src_name, m) in mats if ts_name == s and src_name == "Wind")
                ...
                last_row_in_group = (row == min(nrows - 1, (n_ts - 1) % max_rows_per_col) if (group == ncols_groups - 1)
                                     else row == nrows - 1)
                if not last_row_in_group:
                    ...
                    ax_s.set_xlabel("Hour of day"); ax_w.set_xlabel("Hour of day")
            ...
            fig.suptitle(_wrap(f"{scenario_name}: Average daily curtailment by representative week (Solar | Wind)"),
                         y=0.995)
...
def curtailment_by_region(
    ...
    disp = disp[disp["gen_energy_source"].isin(["Solar", "Wind"])].copy()
    if disp.empty:
        print("[info] No Solar/Wind curtailment rows found in dispatch.csv; nothing to plot.")
        return

    disp["Curtailment_MW"] = pd.to_numeric(disp["Curtailment_MW"], errors="coerce").fillna(0.0)
    ...
    def zone_mats_aggregate_week(zone: str, split: bool):
        ...
        for lab in labels:
            ...
            d["w"] = d["ts_scale_to_period"].fillna(1.0)
            d["mw_w"] = d["Curtailment_MW"] * d["w"]
            prof = (d.groupby(["day_index", "hour_of_day"], as_index=False)
                      .agg(sum_mw_w=("mw_w","sum"), sum_w=("w","sum")))
            prof["Curtailment_MW"] = np.divide(prof["sum_mw_w"], prof["sum_w"],
                                               out=np.zeros_like(prof["sum_mw_w"]), where=prof["sum_w"]>0)
            pvt = (prof.pivot(index="day_index", columns="hour_of_day", values="Curtailment_MW")
                        .reindex(index=range(0,7), columns=range(0,24))).fillna(0.0)
            mat = pvt.values
            ...
            vmin_mw = min(vmin_mw, float(np.nanmin(mat)))
            vmax_mw = max(vmax_mw, float(np.nanmax(mat)))
        if not np.isfinite(vmin_mw) or not np.isfinite(vmax_mw): vmin_mw, vmax_mw = 0.0, 1.0
        if vmax_mw == vmin_mw: vmax_mw = vmin_mw + 1e-9
    ...
    def zone_mats_split_week(zone: str, split: bool):
        ...
        for s in series_list:
            ...
            for lab in labels:
                d = g_ts if lab == "Solar+Wind" else g_ts[g_ts["gen_energy_source"] == lab]
                d = d.groupby(["timestamp", "day_index", "hour_of_day"], as_index=False)["Curtailment_MW"].sum()
                pvt = (d.pivot(index="day_index", columns="hour_of_day", values="Curtailment_MW")
                          .reindex(index=range(0,7), columns=range(0,24))).fillna(0.0)
                mat = pvt.values
    ...
    for zone in target_zones:
        if aggregate == "week":
            ...
            if not mats_mw:
                print(f"[info] Zone {zone}: no curtailment data to plot."); continue
            out_unit, mult = choose_unit(vmax_mw)
            ...
            series_list, mats_mw, vmin_mw, vmax_mw = zone_mats_split_week(zone, split_by_source)
            if not mats_mw:
                print(f"[info] Zone {zone}: no curtailment data to plot."); continue
            out_unit, mult = choose_unit(vmax_mw)
            ...
            if not split_by_source:
                ...
                for i, s in enumerate(series_list):
                    ...
                    row = i % max_rows_per_col
                    ax = axes[row, group]
                    mat = next(m for (ts_name, lab, m) in mats if ts_name == s and lab == "Solar+Wind")
                    im = ax.imshow(mat, aspect="auto", origin="lower", vmin=vmin, vmax=vmax)
                    last_im = im
                    w = float(weight_map.get(s, 1.0)); pct = 100.0 * w / total_w
                    ax.set_ylabel(f"{s}\n{int(w)} wk ({pct:.1f}%)", rotation=0, labelpad=45,
                                  ha="right", va="center", fontsize=FIG_FONT_SIZE-1)
                    ax.set_yticks(range(0, 7)); ax.set_yticklabels([f"{d+1}" for d in range(7)])
                    ax.set_xticks(range(0, 24, 4)); ax.set_xticklabels([str(h) for h in range(0, 24, 4)])

                    # bottom labels only for last used row in each group
                    last_used_row = (min(max_rows_per_col, n_ts - group * max_rows_per_col) - 1)
                ...
                fig.suptitle(_wrap(f"{scenario_name}: {zone} — Curtailment by representative week"), y=0.995)

            else:
                # ---- Solar|Wind paired columns with correct hiding for last partial group ----
                ncol_groups = math.ceil(n_ts / max_rows_per_col)  # number of TS column groups
                nrows = min(max_rows_per_col, n_ts)
                ...
                for i, s in enumerate(series_list):
                    ...
                    # Solar
                    mat_s = next(m for (ts_name, lab, m) in mats if ts_name == s and lab == "Solar")
                    ax_s = axes[row, col_solar]
                    im = ax_s.imshow(mat_s, aspect="auto", origin="lower", vmin=vmin, vmax=vmax)
                    last_im = im

                    # Wind
                    mat_w = next(m for (ts_name, lab, m) in mats if ts_name == s and lab == "Wind")
                    ax_w = axes[row, col_wind]
                    ax_w.imshow(mat_w, aspect="auto", origin="lower", vmin=vmin, vmax=vmax)
                ...
                fig.suptitle(_wrap(f"{scenario_name}: {zone} — Curtailment by representative week (Solar | Wind)"),
                             y=0.995)
