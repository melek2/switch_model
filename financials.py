...
# )
# def graph_system_cost_breakdown(tools):
#     df = tools.get_dataframe("costs_itemized.csv")

#     # Pick value column, filter zeros
#     value_col = "AnnualCost_Real" if "AnnualCost_Real" in df.columns else "AnnualCost_NPV"
...
#         "PM25Costs": "PM2.5 Costs",
#     }
#     df["Component"] = df["Component"].replace(rename_map)

#     # Build grouping column list robustly
#     groupby_raw = "PERIOD" if tools.num_scenarios == 1 else ["PERIOD", "scenario_name"]
#     group_cols = [groupby_raw] if isinstance(groupby_raw, str) else list(groupby_raw)

#     # Aggregate then pivot
#     df_agg = (
#         df.groupby(group_cols + ["Component"], as_index=False, observed=True)[value_col]
#           .sum()
#     )
#     costs = (
#         df_agg.pivot(index=group_cols, columns="Component", values=value_col)
#               .fillna(0)
#               * 1e-9
#     )

#     # Order stacks by their contribution in the last row (if possible)
#     try:
#         last_row = costs.iloc[-1]
#         costs = costs.reindex(columns=last_row.sort_values().index)
#     except Exception:
#         pass

#     # Plot
#     ax = tools.get_axes()
#     ylabel = "Billions of dollars (Real)" if value_col == "AnnualCost_Real" else "Billions of dollars (NPV)"
#     costs.plot(kind="bar", stacked=True, ax=ax, xlabel="Period", ylabel=ylabel)
