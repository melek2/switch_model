...
#     title="Local T&D energy balance over time",
...
#     if len(zcols) >= 2:
#         z1, z2 = zcols[:2]
#         imp = imp.rename(columns={z1: "load_zone", z2: "timepoint_id"})
#     else:
#         imp["load_zone"] = "all"
#         imp["timepoint_id"] = range(len(imp))

#     # Align timestamps if timepoints.csv exists
#     try:
...
#             imp["timestamp"] = imp["timepoint_id"].map(ts_map)
#         else:
#             imp["timestamp"] = imp["timepoint_id"]
#     except Exception:
#         imp["timestamp"] = imp["timepoint_id"]

#     # Merge central import into wide table by zone+timestamp
#     # wide already has load_zone and timestamp
#     mcols = ["load_zone", "timestamp"]
#     if "scenario_name" in wide.columns and "scenario_name" in imp.columns:
#         mcols = ["scenario_name"] + mcols

#     imp_slim = imp[["WithdrawFromCentralGrid", "load_zone", "timestamp"] + (["scenario_name"] if "scenario_name" in imp.columns else [])]
#     df = wide.merge(imp_slim, on=mcols, how="left")

#     # Compute local supply and check balance
#     df["LocalSupply"] = df["ZoneTotalDistributedDispatch"] - df["InjectIntoDistributedGrid"]
#     # Choose a representative zone with highest average demand
#     if tools.num_scenarios == 1:
#         z = df.groupby("load_zone")["zone_demand_mw"].mean().sort_values(ascending=False).index[0]
#         sub = df[df["load_zone"] == z].copy()
#     else:
#         # pick top zone within each scenario for clarity
#         scn = df.groupby("scenario_name")["zone_demand_mw"].mean().sort_values(ascending=False).index[0]
#         tmp = df[df["scenario_name"] == scn]
#         z = tmp.groupby("load_zone")["zone_demand_mw"].mean().sort_values(ascending=False).index[0]
#         sub = df[(df["scenario_name"] == scn) & (df["load_zone"] == z)].copy()
...
#     # Stacked area of LocalSupply and WithdrawFromCentralGrid against demand line
#     sub = sub.sort_values("timestamp")
#     sub.set_index("timestamp", inplace=True)
#     stacks = sub[["LocalSupply", "WithdrawFromCentralGrid"]].fillna(0)
#     stacks.plot.area(ax=ax)
#     sub["zone_demand_mw"].plot(ax=ax, kind="line")
#     ax.set_xlabel("Time")
#     ax.set_ylabel("MW")

#     f = Figure(fig, ax)
#     f.add_note(f"Zone shown: {z}")

# @graph(
#     "local_td_energy_balance_shares",
#     title="Import vs local supply shares by zone",
#     supports_multi_scenario=True
# )
# def graph_local_td_energy_balance_shares(tools):
#     wide = tools.get_dataframe("local_td_energy_balance_wide.csv")
#     imp = tools.get_dataframe("WithdrawFromCentralGrid.csv")
...
#     df["LocalSupply"] = df["ZoneTotalDistributedDispatch"] - df["InjectIntoDistributedGrid"]
#     df["TotalServed"] = df["LocalSupply"].fillna(0) + df["WithdrawFromCentralGrid"].fillna(0)

#     if tools.num_scenarios == 1:
#         shares = df.groupby("load_zone")[["LocalSupply", "WithdrawFromCentralGrid", "TotalServed"]].sum()
#         shares = shares[(shares["TotalServed"] > 0)]
#         shares["Local_share"] = shares["LocalSupply"] / shares["TotalServed"]
#         shares["Central_share"] = shares["WithdrawFromCentralGrid"] / shares["TotalServed"]
#         plot_df = shares[["Local_share", "Central_share"]].sort_values("Central_share")
#         idx = "load_zone"
#     else:
#         shares = df.groupby(["scenario_name", "load_zone"])[["LocalSupply", "WithdrawFromCentralGrid", "TotalServed"]].sum()
#         shares = shares[(shares["TotalServed"] > 0)]
#         shares["Local_share"] = shares["LocalSupply"] / shares["TotalServed"]
#         shares["Central_share"] = shares["WithdrawFromCentralGrid"] / shares["TotalServed"]
#         plot_df = shares[["Local_share", "Central_share"]].sort_values("Central_share")
#         idx = ["scenario_name", "load_zone"]

#     ax = tools.get_axes()
#     plot_df.plot(ax=ax, kind="bar", stacked=True, xlabel=("Load zone" if idx == "load_zone" else "Scenario, Load zone"), ylabel="Share of demand served")
