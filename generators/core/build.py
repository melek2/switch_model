...
#     title="Installed capacity by technology (cumulative view)",
#     supports_multi_scenario=True
# )
# def graph_installed_capacity_by_tech(tools):
#     df = tools.get_dataframe("gen_cap.csv")

#     # Keep needed columns
#     need = ["PERIOD", "gen_tech", "GenCapacity"]
#     need += ["scenario_name"] if tools.num_scenarios > 1 else []
#     df = df[[c for c in need if c in df.columns]].copy()

#     # Aggregate snapshot capacity by period × tech
#     groupby = ["PERIOD", "gen_tech"] if tools.num_scenarios == 1 else ["PERIOD", "scenario_name", "gen_tech"]
#     cap = df.groupby(groupby, as_index=False)["GenCapacity"].sum()

#     # Convert to cumulative by period within each tech (and scenario if present)
#     cap = cap.sort_values(groupby)
#     if tools.num_scenarios == 1:
#         cap["CumCapacity"] = cap.groupby("gen_tech")["GenCapacity"].cummax()
#         idx = ["PERIOD"]
#     else:
#         cap["CumCapacity"] = cap.groupby(["scenario_name", "gen_tech"])["GenCapacity"].cummax()
#         idx = ["PERIOD", "scenario_name"]

#     cap_p = cap.pivot_table(index=idx, columns="gen_tech", values="CumCapacity", aggfunc="last").fillna(0)
#     cap_p = cap_p.reindex(columns=cap_p.iloc[-1].sort_values().index)

#     ax = tools.get_axes()
#     cap_p.plot(ax=ax, kind="bar", stacked=False, xlabel="Period", ylabel="Capacity (MW)")
